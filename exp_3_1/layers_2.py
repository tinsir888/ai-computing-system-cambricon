...
class ConvolutionalLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        height_out = np.floor((height-self.kernel_size)/self.stride+1).astype('int')
        width_out = np.floor((width-self.kernel_size)/self.stride+1).astype('int') 
...
class MaxPoolingLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        height_out = np.floor((self.input.shape[2]-self.kernel_size)/self.stride+1).astype('int')
        width_out = np.floor((self.input.shape[3]-self.kernel_size)/self.stride+1).astype('int')
