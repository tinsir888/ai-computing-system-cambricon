...
class VGG19(object):
    ...
    def load_model(self):
        ...
        for idx in range(self.net.size()):
            if 'conv' in self.net.getLayerName(idx):
                ...
                weight = weight.transpose((3, 2, 0, 1)).reshape(-1).astype(np.float)
            ...
            if 'fc' in self.net.getLayerName(idx):
                ...
                weight = weight.transpose((3, 0, 1, 2)).reshape(-1).astype(np.float)
                bias = bias.reshape(-1).astype(np.float)
                self.net.loadParams(idx, weight, bias, self.filter_quant_params[count])
                count += 1
    ...
    def load_image(self, image_dir):
        ...
        # TODO：调整输入数据的形状
        input_image = np.transpose(input_image, (0, 3, 1, 2)).astype(np.float)
        input_data = input_image.reshape(-1)
