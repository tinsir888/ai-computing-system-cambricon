...
def net(image, type=0):
    ...
    res1 = _residual_block(conv3,3,type=type)
    res2 = _residual_block(res1,3,type=type)
    res3 = _residual_block(res2,3,type=type)
    res4 = _residual_block(res3,3,type=type)
    res5 = _residual_block(res4,3,type=type)
    convt1 = _conv_tranpose_layer(res5,64,3,2,type=type)
...
def _conv_layer(net, num_filters, filter_size, strides, padding='SAME', relu=True, type=0):
    ...
    # TODO：进行卷积计算
    net = tf.nn.conv2d(net,filter=weights_init,padding='SAME',strides=strides_shape)
...
def _conv_tranpose_layer(net, num_filters, filter_size, strides, type=0):
    ...
    # TODO：进行转置卷积计算
    net = tf.nn.conv2d_transpose(value=net,filter=weights_init,output_shape=out_shape, padding='SAME', strides=strides_shape)
...
def _residual_block(net, filter_size=3, type=0):
    ...
    tmp_net=_conv_layer(net,128,filter_size,1,relu=True,padding='SAME',type=type)
    tmp_net=_conv_layer(tmp_net,128,filter_size,1,relu=True,padding='SAME',type=type)
    net=net+tmp_net
    return net
