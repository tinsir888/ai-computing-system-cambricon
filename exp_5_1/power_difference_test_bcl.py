...
def power_difference_op(input_x,input_y,input_pow):
    with tf.Session() as sess:
      ...
      ph_x = tf.placeholder(tf.float32, shape=input_x.shape, name='placeholder_x')
      ph_y = tf.placeholder(tf.float32, shape=input_y.shape, name='placeholder_y')
      ph_z = tf.placeholder(tf.float32, name='placeholder_z')
      out = tf.power_difference(ph_x, ph_y, ph_z)
      return sess.run(out, feed_dict = {ph_x:input_x, ph_y:input_y, ph_z:input_pow})
